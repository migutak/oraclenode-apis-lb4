...
export class RelegationapprovalController {
  constructor(
    @inject('datasources.oracle') public dataSource: OracleDataSource
  ) {}

  //relegationapprovals
  @post('/nodeapi/relegationapprovals/all', {
    responses: {
      '200': spec,
    },
  })
  async relegationapprovals
  ...
  buildSql(request: any) {
    ...
    var fromSql = " from tbl_relegations where status in ('pending')";
